...
# =========================================================
#        ZEX Tunnel V3 — Installer (Optimized, No API)
# =========================================================
...
CFG_DIR="${BASE_DIR}/config"             # Immutable templates live here
...
# Executables expected under BASE_DIR
BIN_TUN="${BASE_DIR}/Waterwall"
BIN_WEB="${BASE_DIR}/web.py"

# Mutable runtime files (generated/modified)
...
# Immutable templates (must exist)
...
# If script directory isn't writable, fall back to /tmp
if ! ( : >"$LOG_FILE" ) 2>/dev/null; then
  LOG_FILE="/tmp/zex-tunnel-install-$(date +'%Y%m%d-%H%M%S').log"
  : >"$LOG_FILE"
fi

# mirror stdout+stderr to terminal + log
...
# Optional trace: TRACE=1 sudo bash script.sh
if [[ "${TRACE:-0}" == "1" ]]; then
  set -x
...
# Multi-port expansion for IR config
...
main(){
  ...
  # Keep the behavior: warn if expected files not there yet (do NOT abort)
  for f in "$BIN_TUN" "$BIN_WEB"; do
    [[ -f "$f" ]] || warn "Expected file not found yet: $f"
  done
